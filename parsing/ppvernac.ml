...
let rec pr_vernac = function
  ...
  | VernacFail (false,v) -> str"Fail" ++ spc() ++ pr_vernac v
  | VernacFail (true,v) -> str"NoAnomaly" ++ spc() ++ pr_vernac v
  ...
  (* Gallina extensions *)
  | VernacBeginSection (loc, id) -> hov 2 (str"Section" ++ pr_opt (fun id -> spc () ++ pr_lident (loc, id)) id )
